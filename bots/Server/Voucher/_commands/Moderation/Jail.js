...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    let msg = await message.reply({embeds: [new genEmbed().setAuthor({ name: uye.user.username, iconURL: uye.user.avatarURL({extension: 'png'})}).setDescription(`Belirtilen ${uye} isimli üyeyi cezalandırmak istediğinize emin misiniz?`)], components: [jailOptions]}).catch(err => {})
    ...
    collector.on('collect', async i => {
        if (i.customId === `onayla`) {
            i.update({embeds: [new genEmbed().setAuthor({ name: uye.user.username, iconURL: uye.user.avatarURL({extension: 'png'})}).setDescription(`Belirtilen ${uye} isimli üyesini hangi sebep ile cezalandırmak istiyorsun?\n${!roller.kurucuRolleri.some(x => message.member.roles.cache.has(x)) && !ayarlar.staff.includes(message.member.id) && !message.member.permissions.has(Discord.PermissionFlagsBits.Adminisrator) ? Number(ayarlar.jailLimit) ? `Kullanılabilir Limit: \`${getLimit.get(message.member.id) || 0} / ${ayarlar.jailLimit}\`` : `` : ``}`)],  components: [
                new ActionRowBuilder().addComponents(
                    new Discord.StringSelectMenuBuilder()
                        .setCustomId(`sebep`)
                        .setPlaceholder(`${uye.user.username} için ceza sebebi belirtin!`)
                        .addOptions(sebeps.filter(x => x.type === 3).map(opt => {
