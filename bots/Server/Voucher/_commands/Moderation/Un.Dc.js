...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    let cezakontrol = await DC.findById(uye.id) 
    if(!cezakontrol) {
    ...
    let cezabilgisi = await Punitives.findOne({ Member: uye.id, Active: true, Type: "DC Cezalandırma" }) 
    ...
    if(await DC.findById(uye.id)) {
        await DC.findByIdAndDelete(uye.id)
    }
    if(uye && uye.manageable) await uye.roles.remove(roller.dcCezalıRolü).catch(err => {});;
