...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    let cezakontrol = await VK.findById(uye.id) 
    if(!cezakontrol) {
    ...
    let cezabilgisi = await Punitives.findOne({ Member: uye.id, Active: true, Type: "VK Cezalandırma" }) 
    ...
    if(await VK.findById(uye.id)) {
        await VK.findByIdAndDelete(uye.id)
    }
    if(uye && uye.manageable) await uye.roles.remove(roller.vkCezalıRolü).catch(err => {});;
