...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    let msg = await message.reply({embeds: [new genEmbed().setAuthor({ name: uye.user.username, iconURL: uye.user.displayAvatarURL({extension: 'png'})}).setDescription(`Belirtilen ${uye} isimli üyeyi **${aktivite}** etkinliğinden cezalandırmak istiyor musun?`)], components: [jailOptions]}).catch(err => {})
    ...
    collector.on('collect', async i => {
        if (i.customId === `onayla`) {
            i.update({
                embeds: [
                    new EmbedBuilder()
                        .setAuthor({ name: uye.user.username, iconURL: uye.user.displayAvatarURL({ extension: 'png' }) })
                        .setDescription(`Belirlenen ${uye} isimli üyesini hangi sebep ile **${aktivite}** etkinliğinden cezalandırmak istiyorsun?`)
                ],
                components: [
                    new ActionRowBuilder().addComponents(
                        new StringSelectMenuBuilder()
                            ...
                            .addOptions(sebeps.filter(x => x.type === 11).map(opt => {
