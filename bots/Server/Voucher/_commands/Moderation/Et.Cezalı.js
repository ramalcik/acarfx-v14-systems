...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    let msg = await message.reply({embeds: [new genEmbed().setAuthor({ name: uye.user.username, iconURL: uye.user.avatarURL({extension: 'png'})}).setDescription(`Belirtilen ${uye} isimli üyeyi etkinliklerden cezalandırmak istiyor musun?`)], components: [jailOptions]}).catch(err => {})
    ...
    collector.on('collect', async i => {
        if (i.customId === `onayla`) {
            i.update({embeds: [new genEmbed().setAuthor({ name: uye.user.username, iconURL: uye.user.avatarURL({extension: 'png'})}).setDescription(`Belirlenen ${uye} isimli üyesini hangi sebep ile etkinliklerden cezalandırmak istiyorsun?`)],  components: [
