...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    let msg = await message.reply({embeds: [new genEmbed().setAuthor({ name: uye.user.username, iconURL: uye.user.avatarURL({extension: 'png'})}).setDescription(`Belirtilen ${uye} isimli üyeyi yayıncı odalarıda cezalandırmak istiyor musun?`)], components: [jailOptions]}).catch(err => {})
    ...
    collector.on('collect', async i => {
        if (i.customId === `onayla`) {
            i.update({embeds: [new genEmbed().setAuthor({ name: uye.user.username, iconURL: uye.user.avatarURL({extension: 'png'})}).setDescription(`Belirlenen ${uye} isimli üyesini hangi sebep ile yayıncı odalarından cezalandırmak istiyorsun?`)],  components: [
                new ActionRowBuilder().addComponents(
                    new Discord.StringSelectMenuBuilder()
                        .setCustomId(`sebep`)
                        .setPlaceholder(`${uye.user.username} için ceza sebebi belirtin!`)
                        .addOptions(sebeps.filter(x => x.type === 9).map(opt => {
        ...
        if (i.customId === `iptal`) {
            msg.delete().catch(err => {})
            return await i.update({ content: `${message.guild.emojiGöster(emojiler.Iptal)} ${uye} isimli üyenin **${aktivite}** cezalandırılma işlemi başarıyla iptal edildi.`, components: [], embeds: [] , ephemeral: true});
        }
    });
