...
module.exports = {
  Isim: "transferler",
  Komut: ["transferlerim"],
  Kullanim: "transferler <@cartel/ID>",
 ...
 .setStyle(ButtonStyle.Danger);
    ...
    Users.findOne({_id: uye.id }, async (err, res) => {
      if (!res) return message.channel.send({embeds: [new genEmbed().setAuthor({ name: uye.user.username, iconURL: uye.user.avatarURL({extension: 'png'})}).setDescription(`${uye} isimli üyenin transfer bilgisi bulunamadı.`)]}).then(x => setTimeout(() => {x.delete()}, 7500))
      if(!res.Transfers) return message.channel.send({embeds: [new genEmbed().setAuthor({ name: uye.user.username, iconURL: uye.user.avatarURL({extension: 'png'})}).setDescription(`${uye} isimli üyenin transfer bilgisi bulunamadı.`)]}).then(x => setTimeout(() => {x.delete()}, 7500))
...
${pages[currentPage - 1].map((value, index) => `\` ${index + 1} \` ${message.guild.members.cache.get(value.Uye) ? message.guild.members.cache.get(value.Uye) : `<@${value.Uye}>`} \`${value.Tutar} ${value.Islem}\` (\`${tarihsel(value.Tarih)}\`)`).join("\n")} `)]}).catch(err => {})
      ...
      collector.on("collect", async (i) => {
        ...
        await i.deferUpdate();
        await curPage.edit({
          embeds: [embed.setFooter({ text: `${ayarlar.serverName ? ayarlar.serverName : message.guild.name} • ${currentPage} / ${pages.length} `, iconURL: message.guild.iconURL({extension: 'png'})}).setDescription(`${uye} isimli üyesinin toplamda \`${res.Transfers.length || 0}\` adet transferi mevcut.
...
${pages[currentPage - 1].map((value, index) => `\` ${index + 1} \` ${message.guild.members.cache.get(value.Uye) ? message.guild.members.cache.get(value.Uye) : `<@${value.Uye}>`} \`${value.Tutar} ${value.Islem}\` (\`${tarihsel(value.Tarih)}\`)`).join("\n")}`)]
      ...
      collector.on("end", () => {
        if(curPage) curPage.edit({
          embeds: [embed.setFooter({ text: `${ayarlar.serverName ? ayarlar.serverName : message.guild.name}`, iconURL: message.guild.iconURL({extension: 'png'})}).setDescription(`${uye} isimli üyesinin toplamda \`${res.Transfers.length || 0}\` adet transferi mevcut.`)],
          components: [],
        }).catch(err => {});
      })
    })
